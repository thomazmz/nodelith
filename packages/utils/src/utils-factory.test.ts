import { FactoryUtils } from './utils-factory'
...
describe('FactoryUtils', () => {
  describe('extractParameters', () => {
    it('should return regular function declaration parameters', () => {
     ...
     const result = FactoryUtils.extractParameters(arrowFunction)
    ...
    it('should return anonymous regular function declaration parameters', () => {
      const result = FactoryUtils.extractParameters((param1: string, param2: number) => {
        throw new Error('Not to be called!')
     })
    ...
    it('should return arrow function declaration parameters', () => {
      ...
      const result = FactoryUtils.extractParameters(regularFunction)
      expect(result.length).toBe(2)
      expect(result[0]).toEqual('param1')
      expect(result[1]).toEqual('param2')
    })
    ...
    it('should return anonymous arrow function declaration parameters', () => {
      const result = FactoryUtils.extractParameters(function regularFunction(param1: string, param2: number) {
        throw new Error('Not to be called!')
      })
